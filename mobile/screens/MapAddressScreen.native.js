...
export default function MapAddressScreen({ navigation }) {
    ...
    (async () => {
      try {
        ...
        if (status !== "granted") {
          ...
          // Fallback to a default location if permission is denied, so the map is still usable.
          ...
          reverseGeocode(fallbackLocation);
        ...
        // Also provide a fallback here in case of GPS errors.
        const fallbackLocation = { latitude: -13.9626, longitude: 33.7741 }; // Lilongwe
        setLocation(fallbackLocation);
        setRegion({ ...fallbackLocation, latitudeDelta: 0.01, longitudeDelta: 0.01 });
        reverseGeocode(fallbackLocation);
  ...
  useEffect(() => {
    ...
    const handler = setTimeout(() => {
      // We can reuse the existing handleSearch logic
      handleSearch();
  ...
  // Debounce reverse geocoding to avoid excessive API calls while panning the map
    ...
    <SafeAreaView style={styles.container}>
      ...
      {region && (
        <MapView
          ...
          provider={PROVIDER_DEFAULT} // Use default provider (OSM)
